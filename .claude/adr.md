...
| **Un lote `RETIRADO` deja de publicarse**, y la accion se llama `lote:retirar` y no `convocatoria:retirar-vehiculo` | Dejarlo visible en gris, como estaba; borrar el lote; distinguir los dos retiros solo por comentario | El retiro **libera** el vehiculo a `DISPONIBLE`, asi que publicar el lote anuncia como en venta algo que ya volvio al catalogo, con su precio de entonces — y reaparece cuando una convocatoria se oculta, se corrige y se republica. Borrarlo dejaria al auditor sin rastro de que estuvo incluido y a que precio, asi que sigue visible en administracion y se filtra solo al publicar (`esLoteOfrecido`, un unico sitio para la rejilla, el detalle y la vista previa). El nombre viejo lo confundia con `vehiculo:retirar`, que es la accion **opuesta**: saca el vehiculo del catalogo para siempre. El tipo de evento conserva su nombre porque la bitacora es append-only |
