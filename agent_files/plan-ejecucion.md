...
| 2026-09-23 | El retiro **del lote** se separa del retiro del vehiculo (`lote:retirar` frente a `vehiculo:retirar`) y un lote `RETIRADO` deja de publicarse: fuera de la rejilla y **404** en su detalle | Dejarlo visible en gris, como estaba: el retiro devuelve el vehiculo a `DISPONIBLE`, asi que el lote anunciaria como en venta algo que ya volvio al catalogo. Tambien se descarto borrar el lote —el auditor perderia que estuvo incluido y a que precio— y renombrar el tipo de evento, que la bitacora append-only no permite |
