...
> `/admin/convocatorias/[id]/vista-publica` y
> `/admin/convocatorias/[id]/vista-publica/lotes/[loteId]` muestran la convocatoria y cada
> vehiculo como los vera un participante, y se abren con `convocatoria:ver-administracion` — la
> puerta que ya le corresponde a quien administra. No tocan el gating de las rutas publicas:
> aquellas siguen exigiendo sus tres condiciones y siguen devolviendo 404.
>
...
>
> **Son dos rutas y no una porque la vista previa de la convocatoria sin la del vehiculo revisa
> la mitad.** Sus tarjetas enlazaban al detalle publico del vehiculo y ahi quien administra
> recibia 404 — por las mismas dos razones. Por eso la rejilla recibe la **ruta desde la que se
> mira** (`rutaBase`) y no el identificador de la convocatoria: cada pantalla enlaza dentro de si
> misma.
>
> Las pantallas renderizan los **mismos componentes** con los **mismos datos**
> (`VistaDeConvocatoria` con `lotesParaCatalogo`, `VistaDeLote` con `loteParaVista`). Eso no es
> economia de codigo: es lo que hace que la vista previa siga siendo una vista previa cuando la
> pantalla publica cambie.
>
> **El bloque de participacion se ve y no se puede pulsar** (`soloLectura`). Ocultarlo dejaria
> fuera la mitad de la pantalla que se quiere revisar; dejarlo vivo invitaria a formarse en una
> fila desde una pantalla de revision. **No es una guarda**: `solicitud:crear` exige permiso de
> venta y gating triple en el servidor, y eso no cambia.
