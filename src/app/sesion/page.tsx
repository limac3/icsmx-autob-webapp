...
// "sin sesion" redirige a login y que "sesion sin rol reconocido" da 403 (no
// 500) — ver agent_files/plan-ejecucion.md. No es una pantalla de negocio.
export const dynamic = "force-dynamic";

const SesionPagina = async () => {
  const sesion = await getSession();
  if (!sesion) redirect("/auth/login");
  if (sesion.roles.length === 0) forbidden();

  const idioma = await obtenerIdiomaDePeticion();
  const diccionario = obtenerDiccionario(idioma);
  const {
    sesion: etiquetas,
    roles: etiquetasRoles,
    tiposParticipante,
  } = diccionario;
  ...
  return (
    <main>
      ...
      <dl>
        ...
        <dt>
          <Text2 renderAs="span">{etiquetas.etiquetaRoles}</Text2>
        </dt>
        <dd>
          <Text2 renderAs="span">
            {sesion.roles.map((rol) => etiquetasRoles[rol]).join(", ")}
          </Text2>
        </dd>
        <dt>
          <Text2 renderAs="span">{etiquetas.etiquetaTipoParticipante}</Text2>
        </dt>
        <dd>
          <Text2 renderAs="span">
            {tiposParticipante[sesion.tipoParticipante]}
          </Text2>
        </dd>
      </dl>
    </main>
